...
/*Here we modified only to support MSE protocal
SShare: means the data are obtained by some other copies, and all copies with SShare are valid,
SUnique: means only this copy obtained the valid value, other data should all be SInvalid
SInvalid: means this copy does not hava valid value


State transfer behavior and related operation:
SInvalid ---[read op]--> SShare:    change state of this copy to SShare, 
change SUnique state of other copy to SShare, if have;
SInvalid ---[write op]--> SUnique:  change state of this copy to SUnique,
change states of all other copies to SInvalid;
SShare ---[read op]--> SShare:      nothing happened;
SShare ---[write op]--> Sunique:    change state of this copy to SUnique,
change states of all other copies to SInvalid;
SUnique ---[read op]---> SUnique:   nothing happened;
SUnique ---[write op]---> SUnique:  nothing happened;
*/

//typedef enum {SInvalid=0, SValid, SOld, SWaiting, SumState} State;
typedef enum {SInvalid=0, SShare, SUnique, SumState} State;

inline char* StateString(State state) {
  switch (state) {
    case SInvalid: return "Invalid";
    case SShare: return "Share";
    case SUnique: return "Unique";
  }
  return NULL;
}
...
/*
 * BlockOnGPUState: record the state of data copy on the GPU
 */
//class BlockOnGPUState : public IHaloRegionObserver {
/*
 * Here BlockOnGPUState is generalized to be PartDataObj class.
 * PartDataObj class means this copy of data only contains part
 * of the whole array and can be generalized to any specific 
...
/*
 * DataObjState is generaized to be IntegDataObj class.
 * IntegDataObj class means this copy of data contains the whole
 * array and can be placed on any accelerators. The same as 
